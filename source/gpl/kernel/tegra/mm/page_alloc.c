...
LX_FREE < CMA_threshold_1 :  allocate based on a ratio 
*/
static unsigned int CMA_threshold_2 = 200000; // 200MB
static unsigned int CMA_threshold_1 = 50000; // 50MB
extern unsigned int VDEC_CMA_size;
#endif
...
void adjust_managed_cma_page_count(struct zone *zone, long count)
{
	...
	*/
    ...
    if(((global_page_state(NR_FREE_PAGES)) << (PAGE_SHIFT - 10)) > CMA_threshold_2){
        ...
        zone->max_try_cma = cma;
        zone->max_try_movable = pageblock_nr_pages/4; // reserve movable for emergent use
    }
#else
	if (movable > cma) {
		zone->max_try_movable =
...
int __meminit init_per_zone_wmark_min(void)
{
  ...
  printk("\n VDEC_CMA_size = %d\n", VDEC_CMA_size);
  if(VDEC_CMA_size > 0){
      CMA_threshold_2 = (VDEC_CMA_size * 1000) + 100000; // VDEC_XC1 size + 100MB
  }
#endif
	return 0;
}
